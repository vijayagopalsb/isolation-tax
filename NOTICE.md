NOTICE
======

Copyright © 2026 Vijayagopal Sivasankara Pillai



INTELLECTUAL PROPERTY NOTICE
---------------------------

The conceptual framework presented in this repository is original work:

1. **"Lateral Isolation"** — the architectural constraint preventing direct peer-to-peer communication between services/modules.

2. **"The Enforcement Gap"** — the identification that existing patterns (Facade, Mediator, API Gateway, ESB) describe intended communication paths but do not enforce them.

3. **The Decision Framework** — criteria for when to enforce vs. skip lateral isolation.

4. **The Dual Enforcement Mechanism** — combining build-time (ArchUnit) and runtime (AOP) enforcement.

First published: 20 July 2026

Author: Vijayagopal Sivasankara Pillai (Vijayagopal S.)

Repository: https://github.com/vijayagopalsb/isolation-tax

Contact: vijayagopal.sb@gmail.com

---

CODE LICENSE
------------
All code in this repository is licensed under the MIT License (see LICENSE file).

CONCEPT LICENSE
---------------
The conceptual framework, methodology, and decision criteria are protected intellectual property.

Attribution is required for:
- Academic citation
- Blog posts or articles referencing this work
- Conference presentations
- Commercial adoption or adaptation

For commercial use, speaking engagements, or consulting, contact the author.

---

This work is protected by copyright law. Unauthorized reproduction or distribution without attribution is prohibited.